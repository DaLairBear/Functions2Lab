...
// CODE HERE
const postTaxPrices = prices.map(price => taxedPrices = price * (1 - .07))
...
// CODE HERE
const postOrderPrices = orders.map(price => taxedPrices = price.price * (1 + price.tax))
...
//   console.log(totalPopulation)
const owner = purchases.filter((person) => {
  if(person.owner === 'Bob'){
    return person.owner
  }
})
console.log(owner)
const bobPurchases = owner.map((price) => {
  return price.price
}
)
console.log(bobPurchases)

const bobsTotal = bobPurchases.reduce((acc, curr) => {
  return acc + curr
})
